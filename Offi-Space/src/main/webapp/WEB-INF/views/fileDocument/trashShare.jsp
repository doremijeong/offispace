...
<body>
        	...
        	<div class="row mb-2">
            	...
            	<div class="col-sm-6">
            		<ol class="breadcrumb float-sm-right">
            			...
            			<li class="breadcrumb-item active">공용 문서 휴지통</li>
            		</ol>
        		</div>
          	</div>
        </div>
    	...
    	<div class="row">
        	...
        	<div class="col-md-4">
            	<div class="card card-info" >
                	<div class="card-header" >
                    	<h3 class="card-title">공용 문서 휴지통 목록</h3>
            ...
            <div class="col-md-8">
            	<div class="card card-info card-outline">
              		<div class="card-header">
                		<h3 id="foldTitle" class="card-title">현재 경로 : [<span id="pathing">${current eq '#' ? '공용/휴지통/' :  current.substring(26) }</span>]</h3>
                    						...
                    						<c:choose>
                    							<c:when test="${doc.icon eq 'fa fa-file  text-warning' }">
                    								...
                    								<tr>
			                    						...
			                    						<td><a href="<%=request.getContextPath()%>/fileDocument/getFile.do?attachSaveName=${doc.realName }&attachSavePath=${doc.parent eq '#' ? 'root':doc.parent}&stand=F">${doc.text }</a></td>
			                    						<td>비공개</td>
                    							...
                    							<c:otherwise>
			                    					...
			                    					<tr>
			                    						...
			                    						<td><a href="<%=request.getContextPath()%>/fileDocument/trashShare.do?current=${doc.id }">${doc.text }</a></td>
			                    						<td>비공개</td>
	...
	function reloading(){
		//location.reload();
		location.href='<%=request.getContextPath()%>/fileDocument/trashShare.do';
	}
    ...
    window.onload=function(){
        ...
        $('#tree').bind('select_node.jstree', function(event, data){
        	...
        	if(data.node.icon!='fa fa-file  text-warning') //폴더
        	{
        		...
        		$.ajax({
    	        	url:'<%=request.getContextPath()%>/fileDocument/getTrashList.do?stand=F',
    	            type:'get',
    	                ...
    	                if(success) //있으면
    	                {
    	                    for(var i of success)
    	                    {
    	                    	if(i.parent==data.node.id)
    	                    	{
    	                    		...
    	                    		{
    	                    			$('#foldList>tbody').append('<tr><td><input type="checkbox" value="'+i.id+'" class="checking"></td> <td><i class="fas fa-folder"></i></td> <td><a href="<%=request.getContextPath()%>/fileDocument/trashShare.do?current='+i.id+'">'+i.text+'</a></td> <td>비공개</td> <td></td> <td>'+i.fileDate+'</td> <td>'+logName+'</td></tr>');
    	                    		}
    	                    	}
    	                    }
    	                }
