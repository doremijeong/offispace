<%@page import="kr.co.offispace.dto.EmployeeVO"%>
...
//     		alert('일정이 등록되었습니다.');
        ...
        var calendar = new Calendar(calendarEl, {
			...
			<c:if test="${loginUser.getChargeCode() eq'R102'}">
		        	customButtons: {
		        	    myCustomButton: {
		        	      text: '일정등록',
		        	      click: function() {
...
<%--         	customButtons: {
          ...
          headerToolbar: {
            left: `<c:if test="${loginUser.getChargeCode() eq'R102'}">myCustomButton</c:if>`,
            center: 'title',
          ...
          eventClick:function(info){
        	  ...
        	  if(info.event.url){
        		  OpenWindow(info.event.url,'일정상세조회','500','600');
        	  }
          },
          eventSources: [
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<div class="row">
					<h5 style="margin: 0; display: inline-block;" onclick="dataGo()">일정등록</h5>
				</div>
			</div>
			<!-- modal header-->


			<form role="form" method="post" action="department/register.do" id="registForm" name="registForm">
						...
						<div class="card card-info card-outline" style="height: auto;">
							...
							<div class="card-body">
								...
								<div class="row d-flex justify-content-center">
									...
									<button type="button" class="modal_close btn btn-default btn-sm" data-dismiss="modal" aria-label="Close" id="btnReset">
...
function dataGo() {
	$('#importantCheck').attr('checked', 'checked');
	$('#scdAlldayCheck').attr('checked', 'checked');
	$('#scdStartDt').val('2022-02-26');
	$('#scdEndDt').val('2022-02-26');
	$('#scdStartTime').attr('disabled', 'disabled');
	$('#scdEndTime').attr('disabled', 'disabled');
	$('#scdTitle').val('대덕인재개발원과 회의');
	$('#scdContent').val('장소 : 대덕인재개발원\n시간 : 오후 1시');
}
</script>
</body>