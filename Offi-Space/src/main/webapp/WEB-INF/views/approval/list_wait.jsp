					...
					<div class="row">
						<div class="col-md-5 ">
								<div class="btn-group" style="height: 32px;">
			                        <button type="button" class="btn btn-info" onclick="allList()" style="font-size: 0.8em;">전체</button>
			                        <button type="button" class="btn btn-info" onclick="apvYet()" style="font-size: 0.8em;">결재할문서</button>
			                        <button type="button" class="btn btn-info" onclick="apvIng()" style="font-size: 0.8em;">결재진행중</button>
			                     </div>
			                     <div class="btn-group" style="height: 32px; margin-left: 5px;">
			                        <button type="button" class="btn btn-default" onclick="hab()" style="font-size: 0.8em;">합의</button>
			                        <button type="button" class="btn btn-default" onclick="apv()" style="font-size: 0.8em;">결재</button>
			                     </div>
						</div>
						<div class="col-md-7 ">
							<div class="card-tools row d-flex justify-content-end">
								...
								<div class="input-group-append input-group-sm" style="width: 300px;">
									...
									<div class="input-group-append">
										<button type="button" class="btn btn-info" style="border-radius:0px 3px 3px 0px;" onclick="list_go(1);">
						...
						<table class="table table-sm table-hover">
							...
							<tbody style="font-size: 0.8em;">
								<c:if test="${empty approvalList}">
									<tr class="text-center">
										<td colspan="8">결재 대기 중인 기안문이 없습니다.</td>
									</tr>
								</c:if>
								<c:if test="${!empty approvalList}">
...
<script>
	...
	function apvYet(){
		 $('input[name="searchType"]').val('apvYet');
		 //$('input[name="searchType"]').attr('value', 'apvYet');
		 list_go(1);
	}

	function apvIng(){
		$('input[name="searchType"]').attr('value','apvIng');
		list_go(1);
	}

	function hab(){
		$('input[name="searchType"]').attr('value','hab');
		list_go(1);
	}

	function apv() {
		$('input[name="searchType"]').attr('value','apv');
		list_go(1);
	}
	...
	function allList() {
		...
		jobForm.find("[name='formKind']").val("");
		jobForm.find("[name='option']").val("");
