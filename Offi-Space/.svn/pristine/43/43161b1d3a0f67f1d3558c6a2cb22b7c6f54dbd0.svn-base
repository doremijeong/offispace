...
<c:set var="refuseList" value="${dataMap.refuseList}" />
...
<body>
	<div class="content-wrapper">
						...
						<table class="table table-sm table-hover">
							...
							<tbody style="font-size: 0.8em;">
								<c:if test="${empty refuseList}">
									<tr class="text-center">
										<td colspan="5">결재 반려된 기안문이 없습니다.</td>
									</tr>
								</c:if>
								<c:if test="${!empty refuseList}">
									<c:forEach items="${refuseList}" var="apv">
										<tr onclick="refuseDetail('${apv.apvCode}')">
											...
											<td class="text-center teamValue">${apv.nowEmp.empName}&nbsp;${apv.nowEmp.positionValue}</td>
										</tr>
									</c:forEach>
								</c:if>
							</tbody>
		...
		function refuseDetail(apvCode) {
			OpenWindow('<%=request.getContextPath()%>/approval/refuse/detail?apvCode='+ apvCode, '', '900', '870');
		}
