...
<c:set var="approvalList" value="${dataMap.requestList}" />
						...
						<table class="table table-sm table-hover">
							...
							<tbody style="font-size: 0.8em;">
								<c:if test="${empty approvalList}">
									<tr class="text-center">
										<td colspan="7">결재 대기 중인 기안문이 없습니다.</td>
									</tr>
								</c:if>
								<c:if test="${!empty approvalList}">
									<c:forEach items="${approvalList}" var="apv">
										<tr>
											...
											<c:if test="${!empty apv.apvTitle}">
												<td onclick="requestDetail('${apv.apvCode}')" style="cursor: pointer;" class="apvTitle"><div class="apvTitle-div" style="width: 300px;">${apv.apvTitle}</div></td>
											</c:if>
											<c:if test="${empty apv.apvTitle}">
												<td onclick="requestDetail('${apv.apvCode}')" style="cursor: pointer;" class="apvTitle"><div class="apvTitle-div" style="width: 300px;">${apv.apvForm.apvFormName} 신청</div></td>
											</c:if>
											<!-- 기안자 -->
