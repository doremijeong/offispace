...
<c:set var="apvCompleteList" value="${dataMap.apvCompleteList}" />
					...
					<div class="table-responsive mailbox-messages">
						<table class="table table-sm table-hover">
							...
							<tbody style="font-size: 0.8em;">
								<c:if test="${empty apvCompleteList}">
									<tr class="text-center">
										<td colspan="7">결재 완료된 기안문이 없습니다.</td>
									</tr>
								</c:if>
								<c:if test="${!empty apvCompleteList}">
									<c:forEach items="${apvCompleteList}" var="apv">
										<tr>
											<!-- 긴급,중요 여부 -->
											...
											<c:if test="${!empty apv.apvTitle}">
												<td onclick="completeDetail('${apv.apvCode}')" style="cursor: pointer;" class="apvTitle"><div class="apvTitle-div" style="width: 300px;">${apv.apvTitle}</div></td>
											</c:if>
											<c:if test="${empty apv.apvTitle}">
												<td onclick="completeDetail('${apv.apvCode}')" style="cursor: pointer;" class="apvTitle"><div class="apvTitle-div" style="width: 300px;">${apv.apvForm.apvFormName} 신청</div></td>
											</c:if>
											<!-- 기안자 -->
...
<script>
	...
	function completeDetail(apvCode){
		OpenWindow('<%=request.getContextPath()%>/approval/completeDetail?apvCode='+ apvCode, '', '900', '870');
	}
