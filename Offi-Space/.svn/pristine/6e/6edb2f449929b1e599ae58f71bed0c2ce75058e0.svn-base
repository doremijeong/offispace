				...
				<div class="modal-header d-flex flex-column" style="padding-bottom: 8px; border-bottom: none;">
					...
					<div class="col-sm-12 d-flex flex-row p-0">
						<div class="col-sm-6 p-0">
							<label onclick="dataGo('1')">받는사람 : </label>
		...
		<div class="modal-content">
			...
			<div class="modal-footer d-flex justify-content-between">
				<div>
					...
					<button type="button" class="btn btn-default btn-md" id="closeFromBtn" data-dismiss="modal" aria-label="Close" onclick="resetMsgModal(this);">
						<span aria-hidden="true">닫기</span>
					</button>
				</div>
				<div>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-header d-flex flex-column" style="padding-bottom: 8px; border-bottom: none;">
					...
					<div class="col-sm-12 d-flex flex-row p-0">
						<div class="col-sm-6 p-0">
							<label onclick="dataGo('2')">받는사람 : </label>
						</div> <!--mailbox-controls-->
...
<!--modal -->
<script>
function dataGo(order) {
	if(order == '1'){
		var str = "김지윤 사원 지금 일일계획보고서 작성도 안되어 있고, 응답도 안하고 뭐하는건가? 근무태도가 왜그러나?";
		$('#msgTextarea').val(str);
		$('p.textCount').html(str.length);
	}
	
	if(order == '2'){
		var str = "지금 당장 얘기 좀 하지......";
		$('#replyMsgTextarea').val(str);
		$('p.textCount').html(str.length);
	}
}
</script>
