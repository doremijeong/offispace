...
<body>
			...
			<div class="card card-info card-outline">
				...
				<div class="card-body">
												...
												<c:if test="${!empty empList}">
													<c:forEach items="${empList}" var="emp">
														<c:if test="${(emp.empId).indexOf('EMP') ne  0 && emp.empCode ne loginUser.empCode}">
															<option value="${emp.empCode}" style="color : black;">
						...
						<div class="col-md-12">
							<div class="form-group">
								<div class="row">
									...
									<div class="col-md-10">
										<input class="form-control form-control-sm" name="mailTitle" value="FW:${mail.mailTitle}">
									</div>
								</div>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<label>To:</label> <c:if test="${!empty mail.mailToEmpList}"><c:set value="0" var="aa"/><c:forEach items="${mail.mailToEmpList}" var="empTo"><c:if test="${aa gt 0}">,&nbsp;</c:if>${empTo.empName}&nbsp;${empTo.positionValue}[${empTo.empId}@offiapce.com]<c:set value="${aa + 1 }" var="aa"/></c:forEach></c:if><br/>
							<label>Cc:</label> <c:if test="${!empty mail.mailRefEmpList}"><c:set value="0" var="aa"/><c:forEach items="${mail.mailRefEmpList}" var="empRef"><c:if test="${aa gt 0}">,&nbsp;</c:if>${empRef.empName}&nbsp;${empTo.positionValue}[${empRef.empId}@offiapce.com]<c:set value="${aa + 1 }" var="aa"/></c:forEach></c:if><c:if test="${empty mail.mailRefEmpList}">(참조자 없음)</c:if><br>
	...
	// 시연용 데이터 입력
 	function datago() {
		...
		var str = "여러분 좋은 아침입니다.<br><br>";
		str += "오늘 아침 이사님께 전달 받은 사항입니다.<br>";
		str += "첨부하신 자료 숙지하여 업무에 반영하시기 바랍니다.<br>";
		str += "숙지에 어려움이 있을 시 회신부탁드립니다.<br><br>";
		str += "그럼 오늘도 좋은 하루 보내세요. <br><br><br>";
		str += $('#content').val();
		$('#content').val(str);
		summernote_go($('textarea#content'), 300);
		$('#pracData').remove();
